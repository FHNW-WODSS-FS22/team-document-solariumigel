...
export default function ParagraphList(props) {
  const { connection, onDeleteParagraph, documentProvider, userProvider } =
    props;
  const [paragraphhtmlItems, setParagraphhtmlItems] = useState(
    props.paragraphItems
  );
  ...
  return (
    <div className="paragraphs">
      {paragraphhtmlItems != null && (
        <ul>{documentProvider.getParagraphItems()}</ul>
      )}
    </div>
  );
}
